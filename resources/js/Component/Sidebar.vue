<template>
    ...
    <div class="text-lg mb-4">
        ...
        <template v-for="menu in menus" :key="menu.name">
            ...
            <div :class="menu.active ? 'active' : ''" class="side-link-container" @click="showNestMenu(menu.name)">
                <i :class="['pi', 'mr-3', menu.icon]" style="font-size:18px"></i>
                <span>{{ menu.name }}</span>
                <i v-if="menu.nested_menus && menu.isOpen" class="pi pi-angle-up" style="font-size:18px"></i>
                <i v-else-if="menu.nested_menus" class="pi pi-angle-down" style="font-size:20px"></i>
            </div>
            </Link>
            <!--  Main Menu Item (No Link) -->
            <div v-else :class="menu.active ? 'active' : ''" class="side-link-container with-link"
...
const menus = ref({
  ...
  category: {
    ...
    mail: {
        name: 'Mails',
        icon: 'pi-inbox',
        active: router.page.url === '/dashboard/mail' ?? false,
        route: route('mail.index'),
    },
    chat: {
        name: 'Chat',
        icon: 'pi-comment',
    ...
    pages: {
        ...
        icon: 'pi-file',
        active: false,
        isOpen: (router.page.url === '/dashboard/hot-slide' ?? false) || (router.page.url === '/dashboard/hot-slide/create' ?? false) || (router.page.url === '/dashboard/banner' ?? false),
        nested_menus: [
            {
                name: 'Hotslides',
            ...
            {
                name: 'Banners',
                active: router.page.url === '/dashboard/banner' ?? false,
                icon: 'pi-plus-circle',
...
.slide-leave-to {
    max-height: 0;
    /* Adjust this value to suit your content */
}
</style>
